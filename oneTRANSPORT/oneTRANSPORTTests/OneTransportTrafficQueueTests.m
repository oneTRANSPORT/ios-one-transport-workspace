//
//  OneTransportTrafficQueueTests.m
//  oneTRANSPORT
//
...
@interface OneTransportTrafficQueueTests : OneTransportBaseTests

@end

@implementation OneTransportTrafficQueueTests
...
- (NSDictionary *)dictSimple {
    ...
    [dict setObject:@"Y" forKey:@"present"];
    [dict setObject:@"12" forKey:@"severity"];
    ...
    [dict setObject:@"Y" forKey:@"present"];
    [dict setObject:@"15" forKey:@"severity"];
    [dict setObject:@"2016-09-15T09:00:00Z" forKey:@"time"];
    [array addObject:dict];
...
- (NSDictionary *)dictSimpleChanged {
    ...
    [dict setObject:@"-0.70586899999999997" forKey:@"fromLongitude"];
    [dict setObject:@"Y" forKey:@"present"];
    [dict setObject:@"19" forKey:@"severity"];
    [dict setObject:@"2016-09-15T10:00:00Z" forKey:@"time"];
    [array addObject:dict];
...
- (void)testEmptyArray {
    ...
    [self.singleton.trafficQueue populateTableWith:[NSArray new] timestamp:[NSDate date]];
    
    NSArray *arrayRetrieved = [self.singleton.trafficQueue retrieveAll:nil];
...
- (void)testAdded {
    ...
    NSArray *arrayCommon = [self.singleton.trafficQueue populateTableWith:dict[@"con"] timestamp:[NSDate date]];
    XCTAssertTrue(arrayCommon.count == 2);
    
    NSArray *arrayRetrieved = [self.singleton.trafficQueue retrieveAll:nil];
    XCTAssertTrue(arrayRetrieved.count == 2);
    
    NSArray *arrayChanges = [self.singleton.trafficQueue checkForChanges:arrayCommon];
...
- (void)testChanged {
    ...
    arrayCommon = [self.singleton.trafficQueue populateTableWith:dict[@"con"] timestamp:[NSDate date]];
    dict = [self dictSimpleChanged];
    arrayCommon = [self.singleton.trafficQueue populateTableWith:dict[@"con"] timestamp:[NSDate dateWithTimeIntervalSinceNow:10]];
    XCTAssertTrue(arrayCommon.count == 2);
...
- (void)testOverwrite {
    ...
    [self.singleton.trafficQueue populateTableWith:dict[@"con"] timestamp:date];
    [self.singleton.trafficQueue populateTableWith:dict[@"con"] timestamp:date];
...
- (void)testRemove {
    ...
    NSDictionary *dict = [self dictSimple];
    [self.singleton.trafficQueue populateTableWith:dict[@"con"] timestamp:[NSDate date]];
    [self.singleton.trafficQueue removeAll];
